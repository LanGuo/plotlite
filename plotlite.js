...
if (!input) {
  ...
  if (lines.length < 3) {
    ...
    const declaration = lines[0].split(/\b/);
    const plotType = declaration[0];
    const plotTitle = declaration.filter(function(n) {
      return n !== " ";
    }).slice(1, declaration.length).join(" ");
